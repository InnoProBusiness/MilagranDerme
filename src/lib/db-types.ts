 ...
 *
 * EXCECAO REGISTRADA (Plano 4, 16/08/2026): o bloco de migrations 1755300000000
 * foi escrito em uma maquina sem Postgres, entao `npm run db:types` nao pode ser
 * rodado para regenerar este arquivo — o codegen exige banco vivo. As adicoes do
 * Plano 4 (canal_venda, movimento_estoque, papel_usuario, tipo_lead, o valor
 * 'em_transito' de pedido_status, as tabelas estoques/estoque_movimentos/
 * usuarios/sessoes/leads e as colunas novas de pedidos e kits) foram escritas A
 * MAO, imitando exatamente o formato do gerador: chaves em ordem alfabetica,
 * `Generated<T>` em toda coluna com DEFAULT, `Timestamp` para timestamptz.
 *
 * ANTES DO DEPLOY: rodar `npm run db:migrate && npm run db:types` contra um banco
 * de verdade e conferir o diff. Qualquer divergencia entre este arquivo e o banco
 * aparece no CI (que roda as migrations de verdade) como erro de typecheck ou de
 * teste — nao como defeito silencioso em producao.
 */
